...
def _get_clustered_network_output(wildcards):
    """Return the clustered network path (aggregation is now inline)."""
    return f"{resources_path}/network/{wildcards.scenario}_network_clustered_demand_renewables_thermal_generators_storage_hydrogen_interconnectors.nc"
