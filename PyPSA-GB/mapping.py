...
def generator_map_plotter(tech, color, marker_scaler, year):
    ...
    l1 = ax.scatter(
        [], [], s=min(sizes) * marker_scaler, edgecolors="black", color=color
    )
    l2 = ax.scatter(
        [], [], s=mean(sizes) * marker_scaler, edgecolors="black", color=color
    )
    l3 = ax.scatter(
        [], [], s=max(sizes) * marker_scaler, edgecolors="black", color=color
    )
...
def network_plotter():
    ...
    for i in range(len(df_lines["bus0"].values)):
        ax.plot(
            [line_coordinates[i][0][0], line_coordinates[i][1][0]],
