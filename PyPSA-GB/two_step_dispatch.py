...
if year > 2020:
    ...
    data_reader_writer.data_writer(
        start, end, time_step, year, year_baseline=year_baseline, scenario=scenario
    )
elif year <= 2020:
...
for i in range(int(batches)):
    ...
    for gen in gen_non_zero:
        network2.generators.p_min_pu.loc[gen] = network.generators.p_min_pu.loc[gen]
...
p_by_carrier = network2.generators_t.p.groupby(
    network2.generators.carrier, axis=1
).sum()

storage_by_carrier = network2.storage_units_t.p.groupby(
    network2.storage_units.carrier, axis=1
).sum()
...
p_by_carrier["Biomass"] = (
    p_by_carrier["Biomass (dedicated)"]
    + p_by_carrier["Biomass (co-firing)"]
    + p_by_carrier["Landfill Gas"]
    + p_by_carrier["Anaerobic Digestion"]
...
p_by_carrier.drop(
    (p_by_carrier.max()[p_by_carrier.max() < 50.0]).index, axis=1, inplace=True
)
...
(p_by_carrier / 1e3).plot(
    kind="area", ax=ax, linewidth=0, color=[colors[col] for col in p_by_carrier.columns]
)
