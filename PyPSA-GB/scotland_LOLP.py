...
def get_rate(
    ...
    for i in range(len(carrier)):
        ...
        if fuel_type != "Battery":
            ...
            max_hours = storage_units[storage_units.index == name][
                "max_hours"
            ].tolist()[0]
...
def convert_type(name, carrier, conversion_dict, power_stations):
    ...
    try:
        ...
        if type0 == "Natural Gas":
            type1 = power_stations[power_stations["Station Name"] == name][
                "Technology"
            ].tolist()[0]
        else:
            type1 = "Excluded"
...
def LOLP(network, year, year_baseline=None, failures_type=None, failures_rate=None):
        ...
        for _ in range(len(failures_type)):
            ...
            if generator_type != "Battery":
                ...
                breakdowwn_rate_battery["Breakdown Rate"] = (
                    1
                    - (1 - breakdowwn_rate_battery["Breakdown Rate"]) * failures_rate[_]
                )
    ...
    generators_rate = pd.DataFrame(
        columns=generators_rate_col, index=generators_rate_index
    )
    generators_rate.apply(
        lambda r: get_rate(
            r,
    ...
    storage_units_rate = pd.DataFrame(
        columns=storage_units_rate_col, index=storage_units_rate_index
    )  ##
    storage_units_rate.apply(
        lambda r: get_rate(
            r,
            storage_units_name_list,
    ...
    pd_stations_w = (
        pd_stations[network.generators.index.tolist()][(pd_rate == 0)]
    ...
    pd_stations[pd_stations_w.columns] = (
        network.generators_t.p_max_pu[pd_stations_w.columns]
        * network.generators.p_nom[pd_stations_w.columns]
    )

    pd_stations_all = pd_stations.copy()

    pd_stations_w = pd_stations[
        pd_stations_w.columns.tolist()
    ]  # [(pd_rate == 0)].dropna(axis=1,how='all').fillna(0)
    ...
    installed_capacity = pd.concat(
        [network.generators.p_nom, network.storage_units.p_nom], axis=0
    )[pd_stations.columns].to_numpy()
...
def Margin(network, pd_stations, year, system_reserve_requirment, year_baseline=None):
    ...
    pd_installed_capacity = pd.concat(
        [network.generators.p_nom, network.storage_units.p_nom], axis=0
    )
    de_rated_capacity = pd_installed_capacity * pd_rate
    margin = (
        de_rated_capacity.sum(axis=1)[0]
...
def split_generators(
    installed_capacity, breakdwon_rate, num=None, value=0, Round=False, Print=False
):
    if num == None:
...
def rate_table(nuclear=True):
    ...
    br_csv = pd.DataFrame.from_dict(
        breakdowwn_rate, orient="index", columns=["Breakdown Rate"]
    )
    ...
    brb_csv = pd.DataFrame.from_dict(
        breakdowwn_rate_battery, orient="index", columns=["Breakdown Rate"]
    )
...
def B6_scaling(year, file_path):
    ...
    B6 = (
        pd_lines.at[17, "s_nom"]
        + pd_lines.at[18, "s_nom"]
        + pd_lines.at[23, "s_nom"]
        + pd_lines.at[24, "s_nom"]
        + pd_lines.at[16, "s_nom"]
    )
    # network.lines
...
def main(
    ...
    year_baseline=2020,
    system_reserve_requirment=1200,
    step=100,
    nuclear=True,
):
    ...
    for bus in buses_scotland:
        ...
        if year >= 2030:
            network.generators.loc[
                ...
                & (network.generators["carrier"] == "CCS Gas"),
                "p_nom",
            ] *= 0.0
            network.generators.loc[
                (network.generators["bus"] == bus)
                & (network.generators["carrier"] == "Natural Gas"),
                "p_nom",
            ] *= 0.0
        # derate hydrogen to 0% of capacity across Scotland
        network.generators.loc[
            (network.generators["bus"] == bus)
            & (network.generators["carrier"] == "Hydrogen"),
            "p_nom",
        ] *= 0.0
        ...
        if year >= 2040:
            network.generators.loc[
                (network.generators["bus"] == bus)
                & (network.generators["carrier"] == "Biomass (dedicated)"),
                "p_nom",
            ] = 0.0
            network.generators.loc[
                (network.generators["bus"] == bus)
                & (network.generators["carrier"] == "CCS Biomass"),
                "p_nom",
            ] = 0.0
    ...
    de_rated_capacity, margin = Margin(
        network,
        pd_stations_all,
        year,
        system_reserve_requirment,
    ...
    print(
        "De_rated_capacity in "
        + str(year)
        + ": "
        + str(de_rated_capacity.sum(axis=1)[0])
    )
    print("System margin:")
    ...
    generators_p_nom_scotland = (
        ...
        .p_nom.groupby(pd.concat([network.generators, network.storage_units]).carrier)
        .sum()
        .sort_values()
    )
    if year > 2020:
    ...
    pd_de_rated_capacity.drop(
        pd_de_rated_capacity[pd_de_rated_capacity < 50].index, inplace=True
    )
    ...
    index_peak_demand = np.where(
        network.loads_t.p_set.sum(axis=1).to_numpy() == peak_demand
    )[0][0]
    pd_peakload_period = pd.DataFrame(
    ...
    for i in range(wdc.shape[0]):
        lolp_p += (
            yy[
                ...
                < peak_demand
                - wdc[i]
                - expect_small_capacity
                + system_reserve_requirment
            ].sum()
            / wdc.shape[0]
        )

    output_lolp.loc["Base case"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole,
    }
    ...
    largest_windfarm = (
        network.generators_t.p_max_pu[
            network.generators[
                network.generators.carrier.isin(["Wind Offshore"])
            ].p_nom.idxmax()
        ]
        * network.generators[
            network.generators.carrier.isin(["Wind Onshore", "Wind Offshore"])
        ].p_nom.max()
    )
    pd_peakload_period["largest windfarm supply"] = largest_windfarm[
        largest_windfarm.index.isin(pd_peakload_period.index)
    ]
    ...
    lole = sum(lolp)
    lole_week = sum(
        lolp_base[: index_peak_demand - 85]
        + lolp[index_peak_demand - 85 : index_peak_demand + 84]
        + lolp_base[index_peak_demand + 84 :]
    )
    ...
    output_lolp.loc["Largest offshore failure"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Largest offshore failure"] = dict(
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Low RES power"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    output_lolp.loc["Gas supply issues"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Gas supply issues"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    output_lolp.loc["Storage failures"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Storage failures"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    output_lolp.loc["Interconnector failure"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Interconnector failure"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    installed_capacity, breakdwon_rate, net_demand, pd_stations_all, pd_stations_w = (
        LOLP(
            ...
            year,
            year_baseline=year_baseline,
            failures_type="Englandconnector",
            failures_rate=0.0,
        )
    )
    large_capacity, large_breakdwon_rate, expect_small_capacity = split_generators(
    ...
    installed_capacity, breakdwon_rate, net_demand, pd_stations_all, pd_stations_w = (
        LOLP(
            ...
            year,
            year_baseline=year_baseline,
            failures_type=["Interconnector", "Englandconnector"],
            failures_rate=0.0,
        )
    )
    large_capacity, large_breakdwon_rate, expect_small_capacity = split_generators(
    ...
    output_lolp.loc["Self-sufficient"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    output_lolp_self.loc["Base case (Self-sufficient)"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Self-sufficient"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    output_lolp_self.loc["Largest offshore failure"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Self_Largest offshore failure"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    output_lolp_self.loc["Low RES power"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Self_Low RES power"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    output_lolp_self.loc["Gas supply issues"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
    ...
    while lole > 0.3:
        ...
        lole_list.append(lole)
        print(f"for {i_*step}MW increased firm capacity, lole is {lole}")
    lole_loop.loc["Self_Gas supply issues"] = dict(
        [[i * step, lole_list[i]] for i in range(i_ + 1)]
    )
    ...
    installed_capacity, breakdwon_rate, net_demand, pd_stations_all, pd_stations_w = (
        LOLP(
            ...
            year,
            year_baseline=year_baseline,
            failures_type=["Battery", "Interconnector", "Englandconnector"],
            failures_rate=0.0,
        )
    )
    large_capacity, large_breakdwon_rate, expect_small_capacity = split_generators(
    ...
    output_lolp_self.loc["Storage failures"] = {
        "peak_lolp": lolp_p,
        "lole": lole,
        "lole_week": lole_week,
    }
...
def main_plot(scenario, year_list):
    ...
    for year in year_list:
        ...
        pd_lolp_self.index = pd.MultiIndex.from_arrays(
            [[year] * 5, pd_lolp_self.index.tolist()]
        )
        ...
        pd_de_rate = pd.concat(
            [
                ...
                pd.read_csv(
                    ...
                    + ".csv",
                    index_col=0,
                ).rename(columns={"de_rated_capacit": year}),
            ],
            axis=1,
        ).fillna(0)
...
def self_sufficient_plot(scenario, year_list, system_reserve_requirment=1200):
    ...
    pd_plot["LOLE for Self-sufficient base case"] = (
        all_lole_loop.loc[:, (slice(None), "Self-sufficient")].loc[0, :].values
    )

    # all_lole_loop.loc[:,(slice(None), 'Self-sufficient')]
    index_1 = (
        all_lole_loop.loc[:, (slice(None), "Self-sufficient")]
        .T.max()[all_lole_loop.loc[:, (slice(None), "Self-sufficient")].T.max() < 3]
        .index[0]
    )
    pd_plot["Base case with additional " + str(index_1) + "MW firm capacity"] = (
        all_lole_loop.loc[index_1, (slice(None), "Self-sufficient")].values
    )
    index_2 = (
        all_lole_loop.loc[:, (slice(None), "Self-sufficient")]
        .T.max()[all_lole_loop.loc[:, (slice(None), "Self-sufficient")].T.max() < 0.3]
        .index[0]
    )
    pd_plot["Base case with additional " + str(index_2) + "MW firm capacity"] = (
        all_lole_loop.loc[index_2, (slice(None), "Self-sufficient")].values
    )
    ...
    for _ in range(len(year_list)):
        if np.isnan(
            ...
            for i in range(len(net_demand)):
                lolp.append(
                    yy[xx < net_demand[i] + system_reserve_requirment - index_1].sum()
                )
            lole = sum(lolp)
            pd_plot.loc[
        ...
        if np.isnan(
            ...
            for i in range(len(net_demand)):
                lolp.append(
                    yy[xx < net_demand[i] + system_reserve_requirment - index_2].sum()
                )
            lole = sum(lolp)
            pd_plot.loc[
                year, "Base case with additional " + str(index_2) + "MW firm capacity"
