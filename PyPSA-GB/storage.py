...
def read_storage_data(year):
    ...
    df2 = df1.loc[
        df1["Technology Type"] == "Pumped Storage Hydroelectricity"
...
def future_storage(FES):
    ...
    df_storage = pd.concat(
        [df_battery, df_compressed_air, df_liquid_air, df_pumped_hydro]
    )
...
def write_storage_units(year, scenario=None, FES=None, networkmodel="Reduced"):
    ...
    if year < 2021:
        df.loc[:, "state_of_charge_initial"] = (
        ...
        df_hydro = df_future_storage[
            df_future_storage.type.str.contains("Pumped Hydro", case=False)
        ]
        df_hydro.columns = df_hydro.columns.map(str)
        date = str(year) + "-01-01 00:00:00"
        df_hydro_capacity = df_hydro[
            df_hydro["Data item"].str.contains("energy capacity", case=False)
        ]
        df_hydro_p_nom = df_hydro[
            df_hydro["Data item"].str.contains("p_nom", case=False)
        ]
        ...
        df_battery = df_future_storage[
            df_future_storage.type.str.contains("Battery", case=False)
        ]
        df_battery.columns = df_battery.columns.map(str)
        date = str(year) + "-01-01 00:00:00"
        df_battery_capacity = df_battery[
            df_battery["Data item"].str.contains("energy capacity", case=False)
        ]
        df_battery_p_nom = df_battery[
            df_battery["Data item"].str.contains("p_nom", case=False)
        ]
        ...
        for bus in df_buses.index.values:
            ...
            store["marginal_cost"] = df_storage_data_by_type.loc[
                "Battery", "marginal_cost"
            ]
            store["efficiency_store"] = df_storage_data_by_type.loc[
                "Battery", "efficiency_store"
            ]
            store["efficiency_dispatch"] = df_storage_data_by_type.loc[
                "Battery", "efficiency_dispatch"
            ]
        ...
        df_compressed_air = df_future_storage[
            df_future_storage.type.str.contains("Compressed Air", case=False)
        ]
        ...
        for bus in df_buses.index.values:
            ...
            store["marginal_cost"] = df_storage_data_by_type.loc[
                "Compressed Air", "marginal_cost"
            ]
            store["efficiency_store"] = df_storage_data_by_type.loc[
                "Compressed Air", "efficiency_store"
            ]
            store["efficiency_dispatch"] = df_storage_data_by_type.loc[
                "Compressed Air", "efficiency_dispatch"
            ]
            store["x"] = df_buses.loc[bus, "x"]
            store["y"] = df_buses.loc[bus, "y"]
            store_list.append(store)
        ...
        df_liquid_air = df_future_storage[
            df_future_storage.type.str.contains("Liquid Air", case=False)
        ]
        ...
        for bus in df_buses.index.values:
            ...
            store["marginal_cost"] = df_storage_data_by_type.loc[
                "Liquid Air", "marginal_cost"
            ]
            store["efficiency_store"] = df_storage_data_by_type.loc[
                "Liquid Air", "efficiency_store"
            ]
            store["efficiency_dispatch"] = df_storage_data_by_type.loc[
                "Liquid Air", "efficiency_dispatch"
            ]
            store["x"] = df_buses.loc[bus, "x"]
            store["y"] = df_buses.loc[bus, "y"]
            store_list.append(store)
