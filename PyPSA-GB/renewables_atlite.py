...
def offshore_wind_farm_timeseries(sites, year, name):
    ...
    nearest = cutout.data.sel(
        {"x": sites.x.values, "y": sites.y.values}, "nearest"
    ).coords
    ...
    power_generation = cutout.wind(
        turbine_type, layout=layout, shapes=cells_generation.geometry
    )
...
def offshore_floating_sites():
    ...
    df_plan["capacity"] = 12.0
    df_plan["No. of Turbines"] = (
        df_plan["Installed Capacity (MWelec)"] / df_plan["capacity"]
    )
...
def offshore_historical_sites(year):
    ...
    df["Installed Capacity (MWelec)"] = (
        df["Turbine Capacity (MW)"] * df["No. of Turbines"]
    )
    df.drop(columns=["Turbine Capacity (MW)", "No. of Turbines"], inplace=True)
    df.rename(columns={"Site Name": "name"}, inplace=True)
    sites = gpd.GeoDataFrame(df).set_index("name")
