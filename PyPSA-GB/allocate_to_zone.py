...
def map_to_zone(df, subzone=None, warm=False):
    ...
    if subzone is not None:
        ...
        json_file = [
            ...
            for i in range(len(json_file))
            if [
                (json_file[j]["properties"]["Name_1"] in subzone)
    ...
    for i in range(len(df)):
        ...
        if (n == 0) & (not np.isnan(df["x"][i])) & (not np.isnan(df["y"][i])):
            if warm:
                print(
                    "point {} is not inside any zone, use the nearest zone instead".format(
                        (df["x"][i], df["y"][i])
                    )
                )
            ...
            if warm:
                print(
                    "Error while allocated for point {}, set to nan value".format(
                        (df["x"][i], df["y"][i])
                    )
                )
