...
def read_tidal_stream(year, scenario, networkmodel):
    ...
    if year in [2025, 2030, 2035, 2040, 2045, 2050]:
        ...
        df_tidal_stream_locations.rename(
            columns={"Lat": "lat", "Lon": "lon", 2050: "p_nom"}, inplace=True
        )
        ...
        dic = {
            "capacities": df_tidal_stream_capacities.loc[year, :],
...
def read_wave_power(year, scenario, networkmodel):
    ...
    if year in [2025, 2030, 2035, 2040, 2045, 2050]:
        ...
        dic = {
            "capacities": df_wave_power_capacities.loc[year, :],
...
def read_floating_wind(year, scenario):
    ...
    if year >= 2025:
        ...
        df_floating_wind_capacities = df_floating_wind_capacities.drop(
            ["Lat", "Lon", "Site ID", "Site Name"]
        )
        ...
        df_floating_wind_capacities = df_floating_wind_capacities / 1000.0
        ...
        # drop the last row as this is a total row
        df_floating_wind_locations.drop(
            df_floating_wind_locations.tail(1).index, inplace=True
        )
        df_floating_wind_locations.rename(
            columns={"Lat": "lat", "Lon": "lon", 2050: "p_nom"}, inplace=True
        )
        ...
        dic = {
            "capacities": df_floating_wind_capacities.loc[year, :],
            "locations": df_floating_wind_locations,
        }
...
def rewrite_generators_for_marine(year, tech, scenario, networkmodel="Reduced"):
    ...
    for generator in dic["locations"].index:
        ...
        if tech == "Tidal stream" or tech == "Wave power":
            df_generators.loc[generator] = {
                "carrier": tech,
                "type": tech,
                "p_nom": dic["locations"].loc[generator, "p_nom"] * 1000.0,
...
def check_consistency_with_p_max_pu():
    ...
    print(
        all(
            ...
            for item in df_generators.loc[
                df_generators["carrier"] == "Wave power"
            ].index
        )
    )
