...
from . import renewables_ninja_data_analysis
...
def read_REPD():
    ...
    # reads csv
    df = pd.read_csv(
        file, encoding="unicode_escape", usecols=fields, lineterminator="\n"
    )
    ...
    df = df.dropna(
        subset=["Installed Capacity (MWelec)", "Technology Type", "Site Name"]
    )
    # reset the index
    ...
    if df["X-coordinate"].isnull().values.any() is True:
        raise Exception(
            "All X-coordinate values must be provided, please fill in missing data in csv file"
        )
    if df["Y-coordinate"].isnull().values.any() is True:
        raise Exception(
            "All Y-coordinate values must be provided, please fill in missing data in csv file"
        )
...
def fix_timeseries_res_for_year(path, year, tech, future):
    ...
    df1.columns = cols
    df1.columns = (
        df1.columns.str.normalize("NFKD")
        .str.encode("ascii", errors="ignore")
        .str.decode("utf-8")
    )
    df1.columns = df1.columns.astype(str).str.replace("\xa0", "")
    ...
    if future is False:
        ...
        for name in range(length):
            ...
            date_operational = pd.to_datetime(
...
def read_tidal_lagoon(year, scenario, fes):
    ...
    if fes == 2021:
        ...
        df_tidal_lagoon_capacities.index = pd.to_datetime(
            df_tidal_lagoon_capacities.index, utc=True
        )
        df_tidal_lagoon_capacities = df_tidal_lagoon_capacities.resample(
            "12MS"
        ).asfreq()
        df_tidal_lagoon_capacities = df_tidal_lagoon_capacities.astype(float)
        df_tidal_lagoon_capacities = df_tidal_lagoon_capacities.interpolate(
    ...
    if fes == 2021:
        ...
        date = year
    dic = {
        "capacities": df_tidal_lagoon_capacities.loc[date, :],
        "locations": df_tidal_lagoon_locations,
    }
...
def read_tidal_stream(year, scenario, fes):
    ...
    df_tidal_stream_capacities = df_tidal_stream_capacities.drop(
        ["Lat", "Lon", "Site ID", "Site Name"]
    )
    ...
    if fes == 2021:
        ...
        df_tidal_stream_capacities.index = pd.to_datetime(
            df_tidal_stream_capacities.index, utc=True
        )
        df_tidal_stream_capacities = df_tidal_stream_capacities.resample(
            "12MS"
        ).asfreq()
        df_tidal_stream_capacities = df_tidal_stream_capacities.astype(float)
        df_tidal_stream_capacities = df_tidal_stream_capacities.interpolate(
            method="linear", limit_direction="forward"
        )
    ...
    if fes == 2021:
        ...
        date = year
    dic = {
        "capacities": df_tidal_stream_capacities.loc[date, :],
        "locations": df_tidal_stream_locations,
    }
...
def read_wave_power(year, scenario, fes):
    ...
    df_wave_power_capacities = df_wave_power_capacities.drop(
        ["Lat", "Lon", "Site ID", "Site Name"]
    )
    ...
    if fes == 2021:
        ...
        df_wave_power_capacities.index = pd.to_datetime(
            df_wave_power_capacities.index, utc=True
        )
    ...
    if fes == 2021:
        ...
        date = year
    dic = {
        "capacities": df_wave_power_capacities.loc[date, :],
        "locations": df_wave_power_locations,
    }
...
def add_marine_timeseries(year, year_baseline, scenario, time_step):
    ...
    df_tidal_lagoon.index = df_tidal_lagoon.index.map(
        lambda x: x.replace(year=year_orig)
    )
    ...
    # pick out required timeseries
    df_tidal_stream.index = df_tidal_stream.index.map(
        lambda x: x.replace(year=year_orig)
    )
    ...
    df_LOPF = pd.concat(
        [df_LOPF, df_tidal_lagoon, df_tidal_stream, df_wave_power], axis=1
    )
    ...
    df_LOPF.columns = (
        df_LOPF.columns.str.normalize("NFKD")
        .str.encode("ascii", errors="ignore")
        .str.decode("utf-8")
    )
    df_LOPF.columns = df_LOPF.columns.astype(str).str.replace("\xa0", "")
...
def aggregate_renewable_generation(start, end, year, time_step):
    ...
    df = df.append(
        {
            ...
            "start_up_cost": df_PV["start_up_cost"][0],
        },
        ignore_index=True,
    )
    ...
    df_new_PV = pd.DataFrame(
        data=[
            df_PV_aggregated_norm.loc[
                df_PV_aggregated_norm.tail(1).index.values
            ].values[0]
        ],
    ...
    df = df.append(
        {
            ...
            "start_up_cost": df_onshore["start_up_cost"][0],
        },
        ignore_index=True,
    )
    # read in the onshore wind time series
    df_onshore_series = renewables_ninja_data_analysis.wind_onshore_corrected_series(
        year
    )

    df_onshore_series = df_onshore_series.loc[start:end]

    df_onshore_aggregated_norm = (
        df_onshore_series.sum(axis=1) / 1000.0 / df_onshore["p_nom"].sum()
    )
    df_onshore_aggregated_norm = pd.DataFrame(
        df_onshore_aggregated_norm, columns=["wind_onshore"]
    )

    # resample to half hourly timesteps
    df_onshore_aggregated_norm = df_onshore_aggregated_norm.resample(freq).interpolate(
        "polynomial", order=1
    )
    # need to add a row at end
    # the data being passed is the values of the last row
    ...
    df_new_offshore = pd.DataFrame(
        data=[
            df_onshore_aggregated_norm.loc[
                df_onshore_aggregated_norm.tail(1).index.values
            ].values[0]
        ],
    ...
    # add to existing dataframe
    df_onshore_aggregated_norm = pd.concat(
        [df_onshore_aggregated_norm, df_new_offshore], ignore_index=True
    )
    ...
    df = df.append(
        {
            ...
            "start_up_cost": df_offshore["start_up_cost"][0],
        },
        ignore_index=True,
    )
    # print(df)
    # read in the onshore wind time series
    df_offshore_series = renewables_ninja_data_analysis.wind_offshore_corrected_series(
        year
    )
    df_offshore_series = df_offshore_series.loc[start:end]

    df_offshore_aggregated_norm = (
        df_offshore_series.sum(axis=1) / 1000.0 / df_offshore["p_nom"].sum()
    )
    df_offshore_aggregated_norm = pd.DataFrame(
        df_offshore_aggregated_norm, columns=["wind_offshore"]
    )
    # resample to half hourly timesteps
    df_offshore_aggregated_norm = df_offshore_aggregated_norm.resample(
        freq
    ...
    df_new_offshore = pd.DataFrame(
        data=[
            df_offshore_aggregated_norm.loc[
                df_offshore_aggregated_norm.tail(1).index.values
            ].values[0]
        ],
    ...
    # add to existing dataframe
    df_offshore_aggregated_norm = pd.concat(
        [df_offshore_aggregated_norm, df_new_offshore], sort=False
    )
    ...
    df = df.append(
        {
            ...
            "start_up_cost": df_hydro_large["start_up_cost"][0],
        },
        ignore_index=True,
    )
    ...
    df_hydro_aggregated_norm = df_series["Bonnington"].values[
        : len(df_offshore_aggregated_norm.index)
    ]
    df_hydro_aggregated_norm = pd.DataFrame(
        df_hydro_aggregated_norm,
        columns=["hydro"],
        index=df_offshore_aggregated_norm.index,
    )
...
def historical_RES_timeseries(year, tech, future=False):
    ...
    if tech == "Wind Offshore" and future is True:
        ...
        # fix the column names
        df_pipeline.columns = (
            df_pipeline.columns.str.normalize("NFKD")
            .str.encode("ascii", errors="ignore")
            .str.decode("utf-8")
        )
        ...
        # fix the column names
        df_future.columns = (
            df_future.columns.str.normalize("NFKD")
            .str.encode("ascii", errors="ignore")
            .str.decode("utf-8")
        )
...
def future_offshore_timeseries(year, year_baseline, scenario, FES):
    ...
    future_capacities_dict = future_offshore_capacity(
    ...
    offshore_cap_scotland_planning = future_capacities_dict[
        "offshore_cap_scotland_planning"
    ]
    offshore_cap_FES = future_capacities_dict["offshore_cap_FES"]

    # first get timeseries for baseline year
    tech = "Wind Offshore"
    df_baseline = historical_RES_timeseries(year_baseline, tech, future=True)[
        "timeseries"
    ]
    ...
    if year_baseline % 4 == 0:
        ...
        if year % 4 != 0:
            ...
            df_baseline = df_baseline[
                ~((df_baseline.index.month == 2) & (df_baseline.index.day == 29))
            ]
            df_pipeline = df_pipeline[
                ~((df_pipeline.index.month == 2) & (df_pipeline.index.day == 29))
            ]
            df_future = df_future[
                ~((df_future.index.month == 2) & (df_future.index.day == 29))
            ]
    ...
    result.columns = result.columns.astype(str).str.replace("\xa0", " ")
    ...
    gen_tech.index = gen_tech.index.astype(str).str.replace("\xa0", " ")
    gen_tech_UC.index = gen_tech_UC.index.str.strip()
    gen_tech_UC.index = gen_tech_UC.index.astype(str).str.replace("\xa0", " ")
...
def future_offshore_capacity(year, year_baseline, scenario, FES):
    ...
    if FES == 2021:
        ...
        df_FES = df_FES / 1000.0

        offshore_cap_FES = df_FES.loc[scenario, year]

    capacity_dict = {
        "offshore_cap_year": offshore_cap_year,
...
def future_RES_capacity(year, tech, scenario, FES):
    ...
    if tech == "Hydro":
        ...
        tech == "Wind Onshore"
        or tech == "Solar Photovoltaics"
        or tech == "Wind Offshore"
    ):
        # how much RES in year
        # read in the renewable generators
        df_res = REPD_date_corrected(year)
        # start with the offshore wind farms
        df_res_tech = df_res.loc[df_res["Technology Type"] == tech].reset_index(
            drop=True
        )
    ...
    if tech == "Wind Onshore":
        ...
        if FES == 2021:
            ...
            df_FES = df_FES[
                df_FES.SubType.str.contains("Solar PV", case=False, na=False)
            ]
            df_FES = df_FES[~df_FES.Variable.str.contains("Generation")]
            cols = [0, 1, 2, 3, 4]
            df_FES.drop(df_FES.columns[cols], axis=1, inplace=True)
        ...
        if FES == 2021:
            ...
            # df_FES.dropna(axis='rows', inplace=True)
            df_FES = df_FES[
                df_FES.SubType.str.contains("Biomass", case=False, na=False)
            ]
            df_FES = df_FES[~df_FES.Variable.str.contains("Generation")]
...
def plot_future_capacities(year):
    ...
    if year > 2020:
        data_reader_writer.data_writer(
            start, end, time_step, year, year_baseline=2020, scenario="Leading The Way"
        )
    if year <= 2020:
