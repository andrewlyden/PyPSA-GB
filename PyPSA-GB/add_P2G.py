...
def add_P2G(year, scenario=None):
    ...
    buses_scotland = (
        ...
        + ["Z1_" + str(_) for _ in range(1, 5)]
        + ["Z" + str(_) for _ in range(2, 7)]
    )
    ...
    df_P2G = df_FES_bb[
        ...
        & (df_FES_bb["Building Block ID Number"] == "Dem_BB009")
    ].copy()
    ...
    if scenario == "Falling Short" or scenario == "Consumer Transformation":
        P2G_nom_scotland = (
            ...
            * (
                df_P2G_year[df_P2G_year.index.isin(buses_scotland)].sum()
                + df_P2G_year[["Direct(SHETL)", "Direct(SPTL)"]].sum()
            )
            + p_available_by_bus[p_available_by_bus.index.isin(buses_scotland)]
        ...
        P2G_nom_rgb = (
            ...
            * (
                df_P2G_year[df_P2G_year.index.isin(buses_rgb)].sum()
                + df_P2G_year["Direct(NGET)"]
            )
            + p_available_by_bus[p_available_by_bus.index.isin(buses_rgb)]
            / p_available_by_bus.sum()
        )
    ...
    pd_G2G_storages = pd.DataFrame(
        {
            "name": [n + " P2G" for n in P2G_nom.index.tolist()],
    ...
    pd.concat([pd_storage_units, pd_G2G_storages.set_index("name")]).to_csv(
        path + "storage_units.csv"
    )
