...
from scripts.FES.gsp_metadata import ensure_gsp_metadata_file
...
def _infer_fes_year_from_path(path: str) -> Optional[int]:
    match = re.search(r"20\d{2}", Path(path).name)
    return int(match.group(0)) if match else None


def resolve_fes_workbook_path(fes_year: int, fes_data_path: Optional[str] = None) -> Path:
    """Locate the raw FES workbook for an FES release year."""

    candidates = sorted(Path("data/FES").glob(f"*{int(fes_year)}*.xlsx"))
    if candidates:
        return candidates[0]

    if fes_data_path:
        data_path = Path(fes_data_path)
        adjacent = sorted(data_path.parent.glob(f"*{int(fes_year)}*.xlsx"))
        if adjacent:
            return adjacent[0]

    raise FileNotFoundError(f"Could not locate raw FES {fes_year} workbook under data/FES")


def _find_ed1_header_row(workbook_path: Path) -> int:
    raw = pd.read_excel(workbook_path, sheet_name="ED1", header=None, nrows=20, dtype=object)
    for idx, row in raw.iterrows():
        if any(_norm_label(value) == "data item" for value in row.values if pd.notna(value)):
            return int(idx)
    raise ValueError(f"Could not find ED1 header row in {workbook_path}")


def _find_column(df: pd.DataFrame, names: list[str]) -> Optional[str]:
    wanted = {_norm_label(name) for name in names}
    for col in df.columns:
        if _norm_label(col) in wanted:
            return col
    return None


def _year_columns(df: pd.DataFrame) -> Dict[int, object]:
    columns = {}
    for col in df.columns:
        year = None
        if isinstance(col, (int, np.integer)):
            year = int(col)
        elif hasattr(col, "year") and 1900 <= int(col.year) <= 2100:
            year = int(col.year)
        elif isinstance(col, str) and re.fullmatch(r"20\d{2}", col.strip()):
            year = int(col.strip())
        if year is not None:
            columns[year] = col
    return columns
...
ED1_CONSUMER_DEMAND_ALIASES = {
    "customer_total": [
        "GB FES Customer Demand: Total",
        "GBFES Customer Demand: Total",
    ],
    "residential": [
        "GBFES Customer Demand: Residential",
    ],
    "industrial_excluding_electrolysis": [
        "GBFES Customer Demand: Total Industrial (Excluding Electrolysis)",
    ],
    "industrial_including_electrolysis": [
        "GBFES Customer Demand: Industrial (Electrolysis needs to be deducted)",
    ],
    "networked_electrolysis_deduction": [
        "GBFES Customer Demand: Deduct Networked Electrolysis from Industrial",
    ],
    "commercial_gross": [
        "GBFES Customer Demand: Underlying Commercial including unmetered demand",
        "GBFES Customer Demand: Commercial with Micro-generation Included",
        "GBFES Customer Demand: Commercial",
    ],
    "direct_transmission_customers": [
        "GBFES Customer Demand: Direct Transmission Customers",
    ],
}


def _ed1_rows(df: pd.DataFrame, data_col: str, aliases: list[str]) -> pd.DataFrame:
    alias_set = {_norm_label(alias) for alias in aliases}
    return df[df[data_col].map(_norm_label).isin(alias_set)]
...
def _ed1_value(
    ...
    rows = _ed1_rows(df, data_col, aliases)
    if rows.empty:
        return None

    scenario_rows = rows[rows[scenario_col].map(_norm_label) == _norm_label(fes_scenario)]
    if scenario_rows.empty:
        return None

    values = pd.to_numeric(scenario_rows[year_col], errors="coerce").dropna()
    if values.empty:
        return None
    return float(values.sum())


def get_ed1_consumer_demand_gwh(
    fes_year: int,
    fes_scenario: str,
    modelled_year: int,
    logger: Optional[logging.Logger] = None,
    fes_data_path: Optional[str] = None,
) -> float:
    """
    Return the ED1/F.53-equivalent national consumer electricity demand in GWh.

    This is gross consumer demand excluding transmission and distribution losses.
    It includes direct transmission customers and does not deduct embedded
    generation, which is modelled explicitly in future scenarios.
    """

    workbook_path = resolve_fes_workbook_path(fes_year, fes_data_path)
    if logger:
        logger.info(f"Reading ED1 consumer demand from {workbook_path}")

    df = pd.read_excel(workbook_path, sheet_name="ED1", header=_find_ed1_header_row(workbook_path), dtype=object)
    data_col = _find_column(df, ["Data item"])
    scenario_col = _find_column(df, ["Scenario", "Pathway", "FES Pathway"])
    period_col = next((col for col in df.columns if "peak" in _norm_label(col) and "annual" in _norm_label(col)), None)
    year_cols = _year_columns(df)

    if data_col is None or scenario_col is None:
        raise ValueError(f"ED1 in {workbook_path} is missing Data item or Scenario/Pathway columns")
    if modelled_year not in year_cols:
        raise ValueError(f"ED1 in {workbook_path} does not contain year {modelled_year}")

    if period_col is not None:
        annual_periods = {"annual [fiscal]", "electricity year", "annual"}
        df = df[df[period_col].map(_norm_label).isin(annual_periods)].copy()

    available_scenarios = sorted(str(value).strip() for value in df[scenario_col].dropna().unique())
    if not any(_norm_label(value) == _norm_label(fes_scenario) for value in available_scenarios):
        raise ValueError(
            f"No ED1 data found for scenario '{fes_scenario}' in FES {fes_year}. "
            f"Available scenarios: {available_scenarios}"
        )

    year_col = year_cols[modelled_year]
    direct_total = _ed1_value(
        df, data_col, scenario_col, year_col, fes_scenario,
        ED1_CONSUMER_DEMAND_ALIASES["customer_total"],
    )
    if direct_total is not None and direct_total > 0:
        if logger:
            logger.info(f"ED1 consumer demand total: {direct_total:.1f} GWh (direct total row)")
        return direct_total

    residential = _ed1_value(
        df, data_col, scenario_col, year_col, fes_scenario,
        ED1_CONSUMER_DEMAND_ALIASES["residential"],
    )
    industrial = _ed1_value(
        df, data_col, scenario_col, year_col, fes_scenario,
        ED1_CONSUMER_DEMAND_ALIASES["industrial_excluding_electrolysis"],
    )
    if industrial is None:
        industrial_gross = _ed1_value(
            df, data_col, scenario_col, year_col, fes_scenario,
            ED1_CONSUMER_DEMAND_ALIASES["industrial_including_electrolysis"],
        )
        electrolysis_deduction = _ed1_value(
            df, data_col, scenario_col, year_col, fes_scenario,
            ED1_CONSUMER_DEMAND_ALIASES["networked_electrolysis_deduction"],
        )
        if industrial_gross is not None and electrolysis_deduction is not None:
            industrial = industrial_gross + electrolysis_deduction

    commercial = _ed1_value(
        df, data_col, scenario_col, year_col, fes_scenario,
        ED1_CONSUMER_DEMAND_ALIASES["commercial_gross"],
    )
    direct = _ed1_value(
        df, data_col, scenario_col, year_col, fes_scenario,
        ED1_CONSUMER_DEMAND_ALIASES["direct_transmission_customers"],
    )

    components = {
        "residential": residential,
        "industrial_excluding_electrolysis": industrial,
        "commercial_gross": commercial,
        "direct_transmission_customers": direct,
    }
    missing = [name for name, value in components.items() if value is None]
    if missing:
        raise ValueError(f"Could not reconstruct ED1 consumer demand for FES {fes_year}; missing {missing}")

    total = sum(float(value) for value in components.values())
    if logger:
        logger.info(
            "ED1 consumer demand components: "
            f"residential={residential:.1f} GWh, "
            f"industrial_excl_electrolysis={industrial:.1f} GWh, "
            f"commercial_gross={commercial:.1f} GWh, "
            f"direct_transmission_customers={direct:.1f} GWh"
        )
        logger.info(f"ED1 consumer demand total: {total:.1f} GWh (component sum)")
    return total

def _osgb36_to_wgs84(easting, northing):
    """Convert British National Grid coordinates to WGS84 lon/lat without PROJ."""
    ...
    lat = lat0 + (northing - n0) / (a * f0)
    for _ in range(8):
        ma = (1 + n + 5 / 4 * n**2 + 5 / 4 * n**3) * (lat - lat0)
        mb = (3 * n + 3 * n**2 + 21 / 8 * n**3) * np.sin(lat - lat0) * np.cos(lat + lat0)
        mc = (15 / 8 * n**2 + 15 / 8 * n**3) * np.sin(2 * (lat - lat0)) * np.cos(2 * (lat + lat0))
        md = 35 / 24 * n**3 * np.sin(3 * (lat - lat0)) * np.cos(3 * (lat + lat0))
    ...
    h = 0.0
    nu_osgb = a / np.sqrt(1 - e2 * np.sin(lat_osgb) ** 2)
    x1 = (nu_osgb + h) * np.cos(lat_osgb) * np.cos(lon_osgb)
    y1 = (nu_osgb + h) * np.cos(lat_osgb) * np.sin(lon_osgb)
    z1 = ((1 - e2) * nu_osgb + h) * np.sin(lat_osgb)

    tx, ty, tz = 446.448, -125.157, 542.060
    s = 20.4894 * 1e-6
    rx = np.radians(0.1502 / 3600)
    ry = np.radians(0.2470 / 3600)
    rz = np.radians(0.8421 / 3600)
    ...
    p = np.sqrt(x2**2 + y2**2)
    lat_wgs = np.arctan2(z2, p * (1 - e2_wgs))
    for _ in range(8):
        nu_wgs = a_wgs / np.sqrt(1 - e2_wgs * np.sin(lat_wgs) ** 2)
        lat_wgs = np.arctan2(z2 + e2_wgs * nu_wgs * np.sin(lat_wgs), p)
    lon_wgs = np.arctan2(y2, x2)

    return np.degrees(lon_wgs), np.degrees(lat_wgs)
...
def normalize_coordinates(df: pd.DataFrame, lon_col: str, lat_col: str, logger: Optional[logging.Logger] = None) -> pd.DataFrame:
    ...
    try:
        ...
        if lon_vals.max() > 100 or lat_vals.max() > 100:
            ...
            # Transform to WGS84 (lat/lon). Some local PROJ installs return
            # non-finite coordinates, so validate and use a deterministic fallback.
            gdf = gdf.to_crs("EPSG:4326")
            converted_lon = gdf.geometry.x.values
            converted_lat = gdf.geometry.y.values
            if (
                not np.isfinite(converted_lon).all()
                or not np.isfinite(converted_lat).all()
                or np.nanmax(np.abs(converted_lon)) > 20
                or np.nanmin(converted_lat) < 40
                or np.nanmax(converted_lat) > 70
            ):
                raise ValueError("PROJ returned invalid WGS84 coordinates")

            # Extract normalized coordinates
            df[lon_col] = converted_lon
            df[lat_col] = converted_lat
            if logger:
        ...
        lon_vals = df[lon_col].values
        lat_vals = df[lat_col].values
        if len(lon_vals) and (np.nanmax(lon_vals) > 100 or np.nanmax(lat_vals) > 100):
            try:
                converted_lon, converted_lat = _osgb36_to_wgs84(lon_vals, lat_vals)
                df[lon_col] = converted_lon
                df[lat_col] = converted_lat
                if logger:
                    logger.warning(
                        "Coordinate normalization via geopandas failed (%s); used internal OSGB36 fallback",
                        e,
                    )
            except Exception as fallback_error:
                if logger:
                    logger.warning(
                        "Error during coordinate normalization: %s; OSGB36 fallback also failed: %s; assuming lat/lon format",
                        e,
                        fallback_error,
                    )
        elif logger:
...
def resolve_gsp_metadata_path(fes_year: int, logger: Optional[logging.Logger] = None) -> Optional[Path]:
    """Return the best available FES GSP metadata file for a release year."""

    metadata_dir = Path("data/network/ETYS")
    requested = metadata_dir / f"fes{fes_year}_regional_breakdown_gsp_info.csv"
    if requested.exists():
        if logger:
            logger.debug("Using GSP metadata file %s for FES year %s", requested, fes_year)
        return requested

    generated = ensure_gsp_metadata_file(fes_year, metadata_dir=metadata_dir, logger=logger)
    if generated is not None and generated.exists():
        if logger:
            logger.info("Generated GSP metadata file %s for FES year %s", generated, fes_year)
        return generated
    ...
    if not available:
        if logger:
            logger.error("No GSP metadata files found in %s", metadata_dir)
        return None

    # GSP coordinates are stable enough for spatial aggregation. Prefer the latest
    # metadata at or before the requested FES year; otherwise use the nearest file.
    previous = [(year, path) for year, path in available if year <= fes_year]
    if previous:
        fallback_year, fallback_path = max(previous, key=lambda item: item[0])
    else:
        fallback_year, fallback_path = min(available, key=lambda item: abs(item[0] - fes_year))

    if logger:
        logger.warning(
            "FES year %s GSP metadata missing; using FES %s metadata from %s",
            fes_year,
            fallback_year,
            fallback_path,
        )
...
def canonical_gsp_name(value) -> str:
    """Normalize GSP names across FES releases for metadata joins."""
...
def gsp_name_variants(value) -> list[str]:
    """Return candidate canonical keys for a FES GSP name."""

    raw = str(value)
    variants = [canonical_gsp_name(raw)]
    variants.extend(canonical_gsp_name(part) for part in re.findall(r"\(([^)]*)\)", raw))
    # Names such as "Walney 2 (Heysham)" should first try "Walney 2".
...
def create_reduced_network_mapping(gsp_names, fes_year: int, logger):
    ...
    try:
        ...
        gsp_df = gsp_df[gsp_df["Name"].notna()].copy()

        requested_gsps = [str(name).strip() for name in gsp_names if pd.notna(name)] if gsp_names is not None else []
        if requested_gsps:
            exact_df = gsp_df[gsp_df["Name"].isin(requested_gsps)].copy()
            exact_df["requested_name"] = exact_df["Name"]
            ...
            matched_rows = []
            exact_names = set(exact_df["requested_name"])
            for requested_name in requested_gsps:
                if requested_name in exact_names:
                    continue
                for key in gsp_name_variants(requested_name):
                    meta_row = metadata_by_key.get(key)
                    if meta_row is not None:
                        matched = meta_row.copy()
                        matched["requested_name"] = requested_name
                        matched_rows.append(matched)
                        logger.debug("Matched GSP name variant %s -> metadata name %s", requested_name, meta_row["Name"])
                        break

            variant_df = pd.DataFrame(matched_rows) if matched_rows else pd.DataFrame(columns=list(gsp_df.columns) + ["requested_name"])
            gsp_df = pd.concat([exact_df, variant_df], ignore_index=True, sort=False)
            gsp_df = gsp_df.drop_duplicates(subset="requested_name", keep="first")
            logger.info(
                "Matched %d/%d requested GSPs to metadata coordinates (%d exact, %d canonical)",
                len(gsp_df),
                len(requested_gsps),
                len(exact_df),
                len(variant_df),
            ...
            gsp_df["requested_name"] = gsp_df["Name"]
        ...
        gsp_for_mapping = gsp_df[['requested_name', 'Longitude', 'Latitude']].copy()
        gsp_for_mapping.columns = ['site_name', 'lon', 'lat']
        gsp_for_mapping = gsp_for_mapping.dropna(subset=['lat', 'lon'])

        bus_coords = buses_df[['name', 'x', 'y']].dropna(subset=['x', 'y']).copy()
        if bus_coords.empty:
            logger.error("No reduced-network bus coordinates available for GSP mapping")
            return None

        logger.info(f"Mapping {len(gsp_for_mapping)} GSPs to reduced network buses using nearest coordinate assignment")
        bus_lats = np.radians(bus_coords['y'].astype(float).to_numpy())
        bus_lons = np.radians(bus_coords['x'].astype(float).to_numpy())
        bus_names = bus_coords['name'].astype(str).to_numpy()
        mapped_rows = []

        for _, site in gsp_for_mapping.iterrows():
            site_lat = np.radians(float(site['lat']))
            site_lon = np.radians(float(site['lon']))
            x = (bus_lons - site_lon) * np.cos((bus_lats + site_lat) / 2.0)
            y = bus_lats - site_lat
            distances_km = np.sqrt(x * x + y * y) * 6371.0
            best_idx = int(np.argmin(distances_km))
            mapped_rows.append({
                'site_name': site['site_name'],
                'bus': bus_names[best_idx],
                'distance_km': float(distances_km[best_idx]),
            })

        mapped_gsps_df = pd.DataFrame(mapped_rows)
        distant_matches = mapped_gsps_df[mapped_gsps_df['distance_km'] > 500.0]
        if not distant_matches.empty:
            logger.warning(
                "%d GSPs mapped more than 500 km from nearest reduced-network bus",
                len(distant_matches),
            )
        # Convert to mapping list format
        mapping_list = []
        all_demand_gsps = set(requested_gsps) if requested_gsps else set(gsp_names)
        mapped_gsps = set()
        ...
        if missing_gsps:
            ...
            fallback_assignments = {
                "Axminster H": "S.W.Penisula",
                "Craig Murrail": "Beauly",
                ...
                "Sandford": "S.W.Penisula",
                "St Johns Wood": "London",
                "Stannahs Gate": "Penwortham",
                "Tynemouth 132kV1kV": "Stella West",
                "Tynemouth 132kV32kV": "Stella West",
            }
...
def get_fes_demand_data(fes_scenario: str, modelled_year: int, logger, fes_year: Optional[int] = None) -> pd.DataFrame:
    """
    Extract future demand for a specific FES scenario and year.

    The FES building-block file provides granular GSP shares through Dem_BB003,
    but Dem_BB003 is materially lower than ESPENI because it is GSP-facing/net
    demand. Future scenarios model distributed generation explicitly, so the
    national annual load target is taken from ED1 consumer demand and allocated
    across GSPs using Dem_BB003 shares.
    """
    try:
        ...
        if fes_year is None:
            fes_year = _infer_fes_year_from_path(fes_data_path)
        if fes_year is None:
            raise ValueError("FES year is required to load ED1 consumer demand")
        ...
        scenario_data = demand_blocks[demand_blocks[scenario_column].map(_norm_label) == _norm_label(fes_scenario)]
        logger.info(f"Found {len(scenario_data)} rows matching scenario '{fes_scenario}'")

        if len(scenario_data) == 0:
            available_scenarios = sorted(str(value).strip() for value in demand_blocks[scenario_column].dropna().unique())
        ...
        if year_column not in scenario_data.columns:
            available_years = [col for col in scenario_data.columns if str(col).isdigit()]
        ...
        gsp_demand = scenario_data.groupby(index_column)[year_column].sum().to_frame()
        gsp_demand = gsp_demand.dropna()
        dem_bb003_total_gwh = float(pd.to_numeric(gsp_demand[year_column], errors='coerce').sum())
        if dem_bb003_total_gwh <= 0:
            raise ValueError(f"{BUILDING_BLOCK_ID} total is zero for {fes_scenario} {modelled_year}")

        ed1_consumer_total_gwh = get_ed1_consumer_demand_gwh(
            int(fes_year),
            fes_scenario,
            modelled_year,
            logger,
        ...
        scale_factor = ed1_consumer_total_gwh / dem_bb003_total_gwh

        demand_data = gsp_demand.copy()
        demand_data[year_column] = pd.to_numeric(demand_data[year_column], errors='coerce') * scale_factor
        demand_data = demand_data.dropna()

        logger.info("Future demand accounting:")
        logger.info(f"  {BUILDING_BLOCK_ID} GSP-facing total: {dem_bb003_total_gwh:.1f} GWh")
        logger.info(f"  ED1 consumer demand target: {ed1_consumer_total_gwh:.1f} GWh")
        logger.info(f"  Scaling {BUILDING_BLOCK_ID} GSP shares by factor: {scale_factor:.4f}")
        logger.info(f"  Final allocated annual demand: {demand_data.sum().iloc[0]:.1f} GWh")
        logger.info(f"Processed demand data shape: {demand_data.shape}")
        logger.info(f"Using index column: {index_column}")
        log_dataframe_info(demand_data, logger, "FES ED1-scaled demand data")
    ...
    try:
        ...
        if is_historical:
            ...
            original_fes_demand = get_fes_demand_data(
                ...
                modelled_year,
                logger,
                get_param_value(getattr(snakemake.params, 'fes_year', None))
            )
