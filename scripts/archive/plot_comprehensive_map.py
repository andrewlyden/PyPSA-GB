...
# Try to access snakemake object, but don't fail if not available
try:
    # Check if snakemake object exists in the global namespace
    snakemake
    SNAKEMAKE_MODE = True
except NameError:
...
def add_network_buses(m: folium.Map, network: pypsa.Network, logger: logging.Logger) -> folium.FeatureGroup:
    ...
    bus_cluster = MarkerCluster(name="Bus Cluster")
...
def add_generators(m: folium.Map, generators_file: str, logger: logging.Logger) -> folium.FeatureGroup:
    ...
    gen_cluster = MarkerCluster(name="Generator Cluster")
...
def add_storage(m: folium.Map, storage_file: str, logger: logging.Logger) -> folium.FeatureGroup:
    ...
    storage_cluster = MarkerCluster(name="Storage Cluster")
...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            generators_file = sys.argv[2]
            storage_file = sys.argv[3]
            interconnectors_file = sys.argv[4]
            output_file = sys.argv[5]
            network_model = sys.argv[6] if len(sys.argv) > 6 else "ETYS"
            scenario = sys.argv[7] if len(sys.argv) > 7 else "HT35"
        elif SNAKEMAKE_MODE:
            # Snakemake mode - use snakemake object
            network_file = snakemake.input.network
            generators_file = snakemake.input.generators
            storage_file = snakemake.input.storage
            interconnectors_file = snakemake.input.interconnectors
            output_file = snakemake.output.map
            network_model = snakemake.wildcards.network_model
            scenario = snakemake.wildcards.scenario
        else:
            raise ValueError("No input arguments provided and not running from Snakemake")
