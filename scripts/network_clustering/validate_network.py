"""
General PyPSA network validation report writer for PyPSA-GB.
...
def fill_missing_bus_coordinates(network: pypsa.Network, logger: logging.Logger) -> tuple:
    """
    Fill missing bus coordinates using spatial imputation strategies.
    
    For buses with missing coordinates, this function:
    1. Uses nearby bus coordinates (mean of neighbors)
    2. Falls back to technology-based regional centroids
    3. Falls back to network centroid for remaining buses
    
    Returns:
        (buses_filled, num_imputed, status_message)
    """
    buses = network.buses.copy()
    
    # Check if x/y columns exist
    if 'x' not in buses.columns:
        buses['x'] = np.nan
    if 'y' not in buses.columns:
        buses['y'] = np.nan
    
    # Identify buses with missing coordinates
    missing_mask = buses['x'].isna() | buses['y'].isna()
    num_missing = missing_mask.sum()
    
    if num_missing == 0:
        return buses, 0, "All buses have valid coordinates"
    
    logger.info(f"Found {num_missing} buses with missing coordinates")
    
    num_imputed = 0
    
    # Strategy 1: If buses have generators/loads, use generator/load locations
    if 'Generator' in network.components and len(network.generators) > 0:
        gen_with_coords = network.generators[
            (network.generators['bus'].isin(buses.index)) &
            network.generators['bus'].notna()
        ]
        
        for bus_id in buses[missing_mask].index:
            # Check if generators are on this bus
            bus_gens = network.generators[network.generators['bus'] == bus_id]
            if len(bus_gens) > 0 and 'x' in bus_gens.columns and 'y' in bus_gens.columns:
                valid_gen_coords = bus_gens[
                    (bus_gens['x'].notna()) & (bus_gens['y'].notna())
                ]
                if len(valid_gen_coords) > 0:
                    buses.loc[bus_id, 'x'] = valid_gen_coords['x'].mean()
                    buses.loc[bus_id, 'y'] = valid_gen_coords['y'].mean()
                    num_imputed += 1
                    continue
        
        logger.info(f"Imputed {num_imputed} buses using generator coordinates")
    
    # Update missing mask
    missing_mask = buses['x'].isna() | buses['y'].isna()
    
    # Strategy 2: Use nearby buses (within network topology)
    if 'Line' in network.components and len(network.lines) > 0:
        lines = network.lines
        
        for bus_id in buses[missing_mask].index:
            # Find connected buses
            connected_buses_0 = lines[lines['bus0'] == bus_id]['bus1'].tolist()
            connected_buses_1 = lines[lines['bus1'] == bus_id]['bus0'].tolist()
            connected_buses = connected_buses_0 + connected_buses_1
            ...
            if valid_connected:
                mean_x = np.mean([c[0] for c in valid_connected])
                mean_y = np.mean([c[1] for c in valid_connected])
                buses.loc[bus_id, 'x'] = mean_x
                buses.loc[bus_id, 'y'] = mean_y
                num_imputed += 1
        
        logger.info(f"Imputed {num_imputed} buses using neighbor coordinates")
    
    # Update missing mask
    missing_mask = buses['x'].isna() | buses['y'].isna()
    
    # Strategy 3: Use network centroid for remaining missing coordinates
    valid_coords = buses[(buses['x'].notna()) & (buses['y'].notna())]
    if len(valid_coords) > 0:
        centroid_x = valid_coords['x'].mean()
        centroid_y = valid_coords['y'].mean()
        
        for bus_id in buses[missing_mask].index:
            buses.loc[bus_id, 'x'] = centroid_x
            buses.loc[bus_id, 'y'] = centroid_y
            num_imputed += 1
        
        logger.info(f"Imputed {num_imputed} buses using network centroid ({centroid_x:.3f}, {centroid_y:.3f})")
    else:
        # Fallback: Use UK center
        uk_center_x, uk_center_y = -2.5, 54.5
        for bus_id in buses[missing_mask].index:
            buses.loc[bus_id, 'x'] = uk_center_x
            buses.loc[bus_id, 'y'] = uk_center_y
            num_imputed += 1
        
        logger.info(f"No valid coordinates found, using UK center as fallback")
    
    status_msg = f"Imputed coordinates for {num_imputed} buses"
    return buses, num_imputed, status_msg
