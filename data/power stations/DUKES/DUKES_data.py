...
def read_DUKES(year):
    ...
    try:
        ...
        df.drop(
            df[
                df["Location Scotland, Wales, Northern Ireland, or English region"]
                == "Northern Ireland"
            ].index,
            inplace=True,
        )
        df.rename(
            columns={
                "Location Scotland, Wales, Northern Ireland, or English region": "Location"
            },
            inplace=True,
        )
...
def geolocation_data(year, df_powerplants):
    ...
    if not year == 2020:
        ...
        df_powerplants.rename(
            index={"Fiddler’s Ferry GT": "Fiddler`s Ferry GT"}, inplace=True
        )
        df_powerplants.rename(
            index={"Fiddler’s Ferry": "Fiddler`s Ferry"}, inplace=True
        )
